...
struct DistDesignGroup {
  ...
  arma::uvec valid_pairs; // Indices into the global 'pairs' matrix
  arma::mat X_pair_sub;
  arma::mat B_pair, invXtX_pair, Xt_pair;
  arma::vec alpha; 
  arma::vec w_sample; 
  std::vector<arma::uvec> perm_blocks; // Global Indices
};

struct Job { arma::uword col_idx; int group_idx; };

// Filter blocks but PRESERVE global indices (for Drop Mode)
static std::vector<arma::uvec> filter_blocks_global(
  ...
  for(const auto& blk : global_blocks) {
    ...
    for(arma::uword idx : blk) {
      if(idx < is_valid.size() && is_valid[idx]) {
        keep.push_back(idx);
      }
    }
    if(keep.size() > 1) { 
      out.push_back(arma::uvec(keep));
    }
  }
  return out;
}
...
static arma::vec get_projection_vector(const arma::mat& X, const arma::vec& contrast) {
  arma::mat invXtX, Xt;
  if (!inv_xtx_safe(X, invXtX, Xt, 0.0)) {
    return arma::zeros(X.n_rows);
  }
  return X * (invXtX * contrast);
}

// Calculate distances for EXPLICIT pairs to ensure alignment with Design Matrix
static arma::vec calc_dist_subset(const arma::mat& M, const arma::uvec& perm, 
  ...
  // Subset columns (genes) and reorder rows (samples) according to permutation
  arma::mat M_sub = M.submat(perm, feat_idx); 
  
  // --- Pearson Logic (Center the subset rows) ---
  if (d_code == 3) { 
    M_sub.each_row( [](arma::rowvec& r){ r -= arma::mean(r); } );
    d_code = 2; // Proceed as Cosine
  }
  ...
  arma::vec norms;
  if (d_code == 2) {
    norms = arma::sqrt(arma::sum(arma::square(M_sub), 1));
  }
  ...
  for (uword k = 0; k < n_pairs; ++k) {
    ...
    double d = 0.0;
    // If either sample in the pair is NaN (ghost sample), result is NaN
    // This propagates naturally via double arithmetic
    
    if (d_code == 0) { // L2
      d = arma::norm(M_sub.row(i) - M_sub.row(j), 2);
    } else if (d_code == 1) { // L1
      d = arma::norm(M_sub.row(i) - M_sub.row(j), 1);
...
Rcpp::List fit_with_focusing(
    ...
    const arma::umat& pairs,           // <--- NEW: Explicit Pairs (0-based)
  ...
  cfg.na_mode = na_mode; cfg.na_weight = na_weight; 
  cfg.alt_code = 0; 
  ...
  for(int i=0; i<n_mats; ++i) {
    mats[i] = Rcpp::as<arma::mat>(M_list[i]);
  }
  
  arma::mat M0 = mats[0];
  arma::uword n = M0.n_rows; 
  
  // Map dist_type: 3 = Pearson (cor), 2 = Cosine, 1 = L1, 0 = L2
  int d_code = (dist_type == "l1") ? 1 : 
    (dist_type == "cosine") ? 2 : 
    (dist_type == "cor" || dist_type == "pearson") ? 3 : 0;
  ...
  for (int k = 0; k < n_mats; ++k) {
    const arma::mat& M = mats[k]; 
    if (M.n_rows != n) stop("All matrices in M_list must have same number of rows.");
    pattern_map[hash_valid_rows(M)].push_back(k);
  }
  
  std::vector<DistDesignGroup> groups; 
  groups.reserve(pattern_map.size());
  std::vector<Job> jobs; 
  jobs.reserve(n_mats);
  int grp_idx = 0;
  ...
  for (auto& kv : pattern_map) {
    ...
    // Identify valid pairs from the provided 'pairs' matrix
    // This ensures we only use pairs that exist in both the global design AND the current data chunk
    ...
    for(uword k=0; k < pairs.n_rows; ++k) {
      // Check if both samples in pair k are valid in this data chunk
    ...
    if (is_drop) {
      ...
      g.X_pair_sub = pair_X.each_col() % arma::sqrt(W_p); 
      
      arma::vec W_s(n); 
      W_s.fill(na_weight); W_s.elem(g.valid_samples).fill(1.0);
    ...
    if (inv_xtx_safe(g.X_pair_sub, g.invXtX_pair, g.Xt_pair, 0.0)) {
      ...
      if (is_drop) {
        g.perm_blocks = filter_blocks_global(raw_blocks, is_valid);
      } else {
        g.perm_blocks = raw_blocks;
      }
      g.valid = true;
    }
    groups.push_back(g);
  ...
  // NEW: Observed Distance Matrix (Global Pairs x Cell Types)
  arma::mat ObsDist(pairs.n_rows, n_mats); ObsDist.fill(datum::nan);
  uword n_rows_obs = ObsDist.n_rows; // for linear indexing
...
#pragma omp parallel for schedule(dynamic)
  for(size_t k=0; k<jobs.size(); ++k) {
    ...
    if (!is_drop) {
      std::vector<uword> ghosts;
      std::vector<uword> valids;
      for(uword i=0; i<n; ++i) {
        if(!arma::is_finite(M_local(i,0))) ghosts.push_back(i);
        else valids.push_back(i);
      }
    ...
    // Store Observed Distances in output matrix
    if (is_drop) {
      // Robust linear indexing to avoid subview.elem() ambiguity
      arma::uvec global_idx = grp.valid_pairs + (mat_idx * n_rows_obs);
      ObsDist.elem(global_idx) = Y_full.elem(grp.valid_pairs); 
    } else {
      ObsDist.col(mat_idx) = Y_full;
    }
    ...
    for (int r = 0; r < n_randomizations; ++r) {
      ...
      arma::vec w_perm(n);
      for(uword i=0; i<n; ++i) w_perm[p[i]] = grp.w_sample[i];
  ...
  return Rcpp::List::create(
    ...
    Rcpp::Named("perm_stats") = PermStats,
    Rcpp::Named("Y") = ObsDist
  );
}