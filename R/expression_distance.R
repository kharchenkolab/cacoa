...
getClusterCountMatrices<-function (count.matrices, groups = NULL, common.genes = TRUE, 
          omit.na.cells = TRUE) 
{
...
esitmate.expression.dsiatnce <- function(count.matrices,samplef,cell.type,dist='JS',min.cells =10){
  ...
  xlist <- lapply(sn(cells),function(ct) {
    #print(ct)
    xd <- ctdm[[ct]]
    nc <- attr(ctdm[[ct]],'cc');
...
plotExpressionDistance <- function(dist,sample.groups,cell.Type=NULL,type='inter'){
  ...
  if (type=='inter'){
    xde <- Rtsne(x,is_distance=TRUE, perplexity=4,max_iter=1e4)$Y
    df <- data.frame(xde); rownames(df) <- rownames(x); colnames(df) <- c("x","y");
